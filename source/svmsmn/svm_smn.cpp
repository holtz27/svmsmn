...
double logpost_v_t( double e, vec l, int T, double alpha, double li, double ls ){
  ...
  double mu = (2 / alpha) * atanh( (2 * mu_e_t - ls - li) / (ls - li) );
...
double rmhmc_v_t(double e_cur, vec l, int fixp, int L, double eps, int T, int &acc, double alpha, double li, double ls ){
    ...
    G_v_t( e_cur, G, inv_G, dG_v, T, alpha, li, ls );
    double pcur =  G * randn( );
    ...
    for (int jgl = 1 ; jgl < L + 1 ; jgl++ ){
    	...
    	G_v_t( theta2a, G, inv_G, dG_v, T, alpha, li, ls );
    	gdmom1 = gradHmom_v_t( inv_G, pb );
    	gdmom2 = gradHmom_v_t( inv_G, pb );
    
    	for(int jfix = 1 ; jfix < fixp + 1 ; jfix++ ){
    		theta2b = theta2a + 0.5 * eps * gdmom1 + 0.5 * eps * gdmom2;
    		G_v_t( theta2b, G, inv_G, dG_v, T, alpha, li, ls );
    		gdmom2 = gradHmom_v_t( inv_G, pb );
    	}
...
double logpost_v_vg( double e, vec l, int T, double alpha, double li, double ls ){
  ...
  double L = 0.0;
  double v = 0.5 * ( (ls - li) * tanh( 0.5 * alpha * e ) + (ls + li) );
  double mu = (2 / alpha) * atanh( (2 * mu_e_vg - ls - li) / (ls - li) );
...
double logpost_v_s( double e, vec l, int T, double alpha, double li, double ls ){
  ...
  double L = 0.0;
  double v = 0.5 * ( (ls - li) * tanh( 0.5 * alpha * e ) + (ls + li) );
  double mu = (2 / alpha) * atanh( (2 * mu_e_s - ls - li) / (ls - li) );
  
  //# log p( l|v )
  L += T * log( v ); 
  L += (v - 1) * sum( log( l ) );
  //# log priori
  L += - 0.5 * ( (e - mu) / s_e_s ) * ( (e - mu) / s_e_s ); 
...
double glogpost_v_s( double e, vec l, double inv_G, double dG_v, int T, double alpha, double li, double ls ){
  ...
  // log p( l|v )
  // Obs.: R::psigamma(v + 1, 0) - R::psigamma(v, 0) = 1 / v 
  grad += T / v + sum( log(l) );
  // chain rule factor
  grad *= dv;
  // log priori 
  grad += - (e - mu) / (s_e_s * s_e_s);
  //# Partial Energy function
  grad += - 0.5 * inv_G * dG_v;
  //# Partial Energy function
  grad += - 0.5 * inv_G * dG_v;
...
int G_v_s( double e, double &G, double &inv_G, double &dG_v, int T, double alpha, double li, double ls ){
  ...
  // log verossimilhança
  // Obs.: R::psigamma(v + 1, 1) - R::psigamma(v, 1) = - 1 / (v*v)
  G += - dv * dv * T * ( - 1 / (v*v) );
  // lof priori
  G += 1 / (s_e_s * s_e_s);  
  ...
  // dG_v
  dG_v += - 2 * dv * dv2 * T * ( - 1 / (v*v) );
  dG_v += ( dv * dv * dv ) * T * ( 2 / (v*v*v) );
