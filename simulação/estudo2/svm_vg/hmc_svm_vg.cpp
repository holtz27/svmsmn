...
List hmc_svm_vg(int N, 
  ...
  for(int it = 1 ; it < N + 1 ; it ++){
    ...
    theta_cur = hmc_theta( theta_cur, h_cur, L_theta, eps_theta, T, acc_theta, cont_theta, inv_M_theta, k_theta, prec_theta );
    b_cur = hmc_b( b_cur, h_cur, l_cur, L_b, eps_b, T, y_T , acc_b, cont_b, inv_M_b, k_b, prec_b );
    h_cur = hmc_h( h_cur, theta_cur, b_cur, l_cur, L_h, eps_h, T, y_T, acc_h );
    v_cur = hmc_v( v_cur, l_cur, L_v, eps_v, T, acc_v, alpha, li, ls, 1 / M_v, cont_v, k_v, prec_v );
