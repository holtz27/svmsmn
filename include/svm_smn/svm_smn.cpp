...
//#############################################################################
//######## mu_star
double logpost_mu(double mu, vec theta1, vec h, int T){
  
  double L = 0.0; 
  //double mu = theta[0];
  double phi = tanh( theta1[0] ); 
  double sigma = exp( theta1[1] );
...
double glogpost_mu(double mu, vec theta1, vec h, int T){
  //theta = (mu, w, gama)
  //double mu = theta[0];
  double phi = tanh( theta1[0] ); 
  double sigma = exp( theta1[1] );
  ...
  // gradiente mu
  grad += (1 - phi * phi ) * (h[0] - mu) / (sigma * sigma); 
  for( int j = 1 ; j < T ; j++ ){
    grad += (1 - phi) / (sigma * sigma) * ( h[j] - mu - phi * (h[j-1] - mu) );
  }
  
  //priori
  grad += - (mu - mu_0) / (s_0 * s_0);
...
double hmc_mu(double mu_cur, vec theta1, vec h, int L, double eps, int T, int &acc){
    ...
    double phi = tanh( theta1[0] ); 
    double sigma = exp( theta1[1] );
    double M = ( ( 1 - phi ) * ( 1 - phi ) * ( T - 1 ) + ( 1 - phi * phi ) ) / ( sigma * sigma ) + ( 1.0 / ( s_0 * s_0 ) );

    double pcur = sqrt( M ) * randn( );
    ...
    //double auxp = pcur.t() * pcur;
    Hm1 = - logpost_mu(mu_cur, theta1, h, T) + 0.5 * pcur * pcur / M;
    //int jlp=1;
    double pa = pcur;
    double pb = pa;
    double pc = pb;
    ...
    for (int jlp = 1 ; jlp < L + 1 ; jlp++ ){
    	...
    	pb = pa + 0.5 * eps * glogpost_mu( vHa, theta1, h, T ); 
    	vHb = vHa + eps * pb / M;
    	pb = pb + 0.5 * eps * glogpost_mu( vHb, theta1, h, T );
    	vHa = vHb;
    	pa = pb;
    ...
    //auxp = pc.t() * pc;
    Hmf = - logpost_mu( vHb, theta1, h, T) + 0.5 * pb * pb / M;
    aMH = std::min( 1.0, exp( - Hmf + Hm1 ) );
    uMH = randu( );
...
//#############################################################################
//######## theta1 = f(phi, sigma)
double logpost_theta1(vec theta1, double mu, vec h, int T){
  //theta = (mu, w, gama)
  double L = 0.0; 
  //double mu = theta[0];
  double phi = tanh( theta1[0] ); 
...
vec glogpost_theta1(vec theta1, double mu, vec h, mat inv_G, mat dG_omega, mat dG_gamma, int T){
  //theta = (mu, w, gama)
  //double mu = theta[0];
  double phi = tanh( theta1[0] ); 
  double sigma = exp( theta1[1] );
   
  vec grad = zeros<vec>(2, 1); 
  ...
  //gradiente w
  grad[0] += - phi + phi * ( 1 - phi * phi ) * (h[0] - mu) * (h[0] - mu) / ( sigma * sigma );
  for( int j = 1 ; j < T ; j++ ){
    grad[0] += (1 - phi * phi) / (sigma * sigma) * ( h[j] - mu - phi * (h[j-1] - mu) ) * ( h[j-1] - mu );
  }
  //priori
  grad[0] += (a_phi - 1) * (1 - phi);
  grad[0] += - (b_phi - 1) * (1 + phi);
  //jacobiano
  grad[0] += - 2 * phi;
  
  // gradiente gama
  grad[1] += - T + (1 - phi * phi ) * (h[0] - mu) * (h[0] - mu) / ( sigma * sigma ); 
  for( int j = 1 ; j < T ; j++ ){
    grad[1] += ( h[j] - mu - phi * (h[j-1] - mu) ) * ( h[j] - mu - phi * (h[j-1] - mu) ) / ( sigma * sigma );
  }
  // priori
  grad[1] += - 2 * (a_s + 1) + 2 * b_s / ( sigma * sigma );
  // jacobiano
  grad[1] += 1;
  
  // Partial Energy function
  //grad[0] += ( -0.5 * trace( inv_G * dG_mu ) );
  grad[0] += ( -0.5 * trace( inv_G * dG_omega ) );
  grad[1] += ( -0.5 * trace( inv_G * dG_gamma ) );
  
  return - grad;
}
int G_theta1(vec theta1, mat &G, mat &inv_G, mat &dG_omega, mat &dG_gamma, int T){
    ...
    inv_G = zeros<mat>(2, 2);
    //dG_mu = zeros<mat>(2, 2);
    dG_omega = zeros<mat>(2, 2);
    dG_gamma = zeros<mat>(2, 2);

    //G(0, 0) = ( ( 1 - phi ) * ( 1 - phi ) * ( T - 1 ) + ( 1 - phi * phi ) ) / ( sigma * sigma ) + ( 1.0 / ( s_0 * s_0 ) );
    G(0, 0) = 2 * phi * phi + ( T - 1 ) * ( 1 - phi * phi ) + ( a_phi + b_phi ) * ( 1 - phi * phi );
    G(0, 1) = 2 * phi;
    G(1, 0) = G(0, 1);
    G(1, 1) = 2 * T + 4 * b_s / ( sigma * sigma );
    ...
    //dG_omega(0, 0) = ( -2 * ( 1 - phi * phi ) / ( sigma * sigma ) ) * ( ( T - 1 ) * ( 1 - phi ) + phi );
    dG_omega(0, 0) = 2 * phi * ( 1 - phi * phi ) * ( 2 - T + 1 - a_phi - b_phi );
    dG_omega(0, 1) =  2 * ( 1 - phi * phi );
    dG_omega(1, 0) = dG_omega(0, 1);

    //dG_gamma(0, 0) = - ( 2 / ( sigma * sigma ) ) * ( ( T - 1 ) * ( 1 - phi ) * ( 1 - phi ) + 1 - phi * phi );
    dG_gamma(1, 1) = - 8 * b_s / ( sigma * sigma );
    
    return 1;
    
}
vec nuH_theta1( mat dG_omega, mat dG_gamma, mat inv_G, vec p ){
    vec iGp = inv_G * p;
    //vec aux1 = iGp.t() * dG_mu * iGp;
    vec aux2 = iGp.t() * dG_omega * iGp;
    vec aux3 = iGp.t() * dG_gamma * iGp;
    vec nuH = zeros<vec>(2, 1);
    nuH[0] = aux2[0];
    nuH[1] = aux3[0];
    //nuH[2] = aux3[0];
...
vec rmhmc_theta1(vec theta1_cur, double mu, vec h, int fixp, int L, double eps, int T, int &acc){
  
    //vec theta1p = zeros<vec>(3, 1);
    vec theta1 = zeros<vec>(2, 1);
    mat inv_G = zeros<mat>(2, 2);
    //mat dG_mu = zeros<mat>(3, 3);
    mat dG_omega = zeros<mat>(2, 2);
    mat dG_gamma = zeros<mat>(2, 2);
    mat G = zeros<mat>(2, 2);
    int gcond = 0;
    /****************************************************************/
    gcond = G_theta1(theta1_cur, G, inv_G, dG_omega, dG_gamma, T);
    if( gcond == -1 ) return theta1_cur;
    vec pcur = chol( G ).t() * mvrgaussian( 2 );
    /****************************************************************/
    vec aux = pcur.t() * inv_G * pcur;
    vec pa = zeros<vec>(2, 1);
    vec pb = zeros<vec>(2, 1);
    vec pc = pcur;
    vec gdpth1( 2 );
    vec gdmom1( 2 );
    vec gdmom2( 2 );
    ...
    double uMH = 0.0;
    double aMH = 0.0;
    double HMtheta1 = -logpost_theta1( theta1_cur, mu, h, T ) + 0.5 * log( det(G) ) + 0.5 * aux[0];
    double HMthetaf = 0.0;

    vec gradHMCth1 = zeros<mat>(2, 1);
    vec theta1c = theta1_cur;
    vec theta1a;
    vec theta1b;
    theta1a = theta1c;
    pa = pc;
    ...
    for( int jgl = 1 ; jgl < L + 1 ; jgl++ ){
	    ...
	    gdpth1 = glogpost_theta1( theta1a, mu, h, inv_G, dG_omega, dG_gamma, T);
    	gradHMCth1 = gdpth1 - 0.5 * nuH_theta1( dG_omega, dG_gamma, inv_G, pa );
      //cout << gdpth1 << ' ' << gradHMCth1 << endl;
    	/****************************************/
    	/***** pn+1/2 *****************************/
    	...
    	for( int jfix = 1; jfix < fixp + 1 ; jfix++ ){
        	pb = pa - 0.5 * eps * gradHMCth1;
        	gradHMCth1 = gdpth1 - 0.5 * nuH_theta1( dG_omega, dG_gamma, inv_G, pb );
          //cout << gradHMCth1 << endl;
    	}
    
    	/**********************************************************/
    	gcond = G_theta1( theta1a, G, inv_G, dG_omega, dG_gamma, T );
    	if( gcond == -1 ) return theta1_cur;
      gdmom1 = gH_mom( inv_G, pb);
    	gdmom2 = gH_mom( inv_G, pb);
    	/****************************************************/
    	...
    	for( int jfix = 1; jfix < fixp + 1; jfix ++ ){
        	...
        	theta1b = theta1a + 0.5 * eps * gdmom1 + 0.5 * eps * gdmom2;
        	gcond = G_theta1( theta1b, G, inv_G, dG_omega, dG_gamma, T );
          if( gcond == -1 ) return theta1_cur;
        	gdmom2 = gH_mom( inv_G, pb );
        
    	}
    /*************************************************************/
    	gdpth1 = glogpost_theta1( theta1b, mu, h, inv_G, dG_omega, dG_gamma, T);
    	gradHMCth1 = gdpth1 - 0.5 * nuH_theta1( dG_omega, dG_gamma, inv_G, pb );
      //cout << gdpth1 << ' ' << gradHMCth1 << endl;
    ...
    aux = pb.t() * inv_G * pb;
    HMthetaf = -logpost_theta1( theta1b, mu, h, T ) + 0.5 * log( det( G ) ) + 0.5 * aux[ 0 ];
    uMH = randu( );
    aMH = std::min( 1.0, exp( -HMthetaf + HMtheta1 ) );
...
double logpost_v_t( double e, vec l, int T, double alpha, double li, double ls ){
  ...
  double mu = mu_e_t; //(2 / alpha) * atanh( (2 * mu_e_t - ls - li) / (ls - li) );
  
  //# log p( l|v )
  L += 0.5 * T * v * log( 0.5 * v  ) - T * log( tgamma( 0.5 * v )  );
  L += 0.5 * v * sum( log( l ) - l );
  //# log priori
  L += - 0.5 * ( (e - mu) / s_e_t ) * ( (e - mu) / s_e_t ); 
...
int G_v_t( double e, double &G, double &inv_G, double &dG_v, int T, double alpha, double li, double ls ){
...
double rmhmc_v_t(double e_cur, vec l, int fixp, int L, double eps, int T, int &acc, double alpha, double li, double ls ){
    ...
    //if( std::isnan( e_cur ) ) cout << "v= " << e_cur << " não é numerico!!!" << endl;
    double e = 0.0;
    double inv_G = 0.0;
    double dG_v = 0.0;
    double G = 0.0;
    ...
    gcond = G_v_t( e_cur, G, inv_G, dG_v, T, alpha, li, ls );
    ...
    double HMtheta1 = - logpost_v_t( e_cur, l, T, alpha, li, ls ) + 0.5 * log(  G  ) + 0.5 * aux;
    double HMthetaf = 0.0;

    double gradHMCth2 = 0.0;
    //double theta2c = e_cur;
    double theta2a = e_cur;
    double theta2b = 0.0;
    pa = pc;
    
    // Generalized Leapfrog Integrater
    for (int jgl = 1 ; jgl < L + 1 ; jgl++ ){

    	gdpth2 = glogpost_v_t( theta2a, l, inv_G, dG_v, T, alpha, li, ls );
    	gradHMCth2 = gdpth2 - 0.5 * nuH_v_t(dG_v, inv_G, pa);

    	for(int jfix = 1 ; jfix < fixp + 1 ; jfix++ ){
    		pb = pa - 0.5 * eps * gradHMCth2;
     		gradHMCth2 = gdpth2 - 0.5 * nuH_v_t( dG_v, inv_G, pb);
    	}
    
    	gcond = G_v_t( theta2a, G, inv_G, dG_v, T, alpha, li, ls );
    	if( gcond == -1 ) return e_cur;

    	gdmom1 = gradHmom_v_t( inv_G, pb );
    	gdmom2 = gradHmom_v_t( inv_G, pb );
    
    	for(int jfix = 1 ; jfix < fixp + 1 ; jfix++ ){
    		theta2b = theta2a + 0.5 * eps * gdmom1 + 0.5 * eps * gdmom2;
    		gcond = G_v_t( theta2b, G, inv_G, dG_v, T, alpha, li, ls );
        if( gcond == -1 ) return e_cur;
    		gdmom2 = gradHmom_v_t( inv_G, pb );
    	}
    
    	gdpth2 = glogpost_v_t( theta2b, l, inv_G, dG_v, T, alpha, li, ls );
    	gradHMCth2 = gdpth2 - 0.5 * nuH_v_t( dG_v, inv_G, pb);
     	pb = pb - 0.5 * eps * gradHMCth2;
    	pa = pb;
    	theta2a = theta2b;
    ...
    HMthetaf = - logpost_v_t( theta2b, l, T, alpha, li, ls ) + 0.5 * log(  G  ) + 0.5 * aux;
    uMH = randu( distr_param( 0, 1 ) );

    aMH = std::min( 1.0, exp( - HMthetaf + HMtheta1 ) );

    if( std::isnan( theta2b ) ){
      e += e_cur;
    }else{
      if( uMH < aMH ){
        e += theta2b;
...
double logpost_v_vg( double e, vec l, int T, double alpha, double li, double ls ){
  ...
  double L = 0.0;
  double v = 0.5 * ( (ls - li) * tanh( 0.5 * alpha * e ) + (ls + li) );
  double mu = mu_e_vg; //(2 / alpha) * atanh( (2 * mu_e_vg - ls - li) / (ls - li) );
  
  //# log p( l|v )
  L += 0.5 * T * v * log( 0.5 * v  ) - T * log( tgamma( 0.5 * v )  );
  L -= 0.5 * v * sum( log( l ) + 1 / l );
  //# log priori
  L += - 0.5 * ( (e - mu) / s_e_vg ) * ( (e - mu) / s_e_vg ); 
...
int G_v_vg( double e, double &G, double &inv_G, double &dG_v, int T, double alpha, double li, double ls ){
...
double rmhmc_v_vg(double e_cur, vec l, int fixp, int L, double eps, int T, int &acc, double alpha, double li, double ls ){
    ...
    gcond = G_v_vg( e_cur, G, inv_G, dG_v, T, alpha, li, ls );
    if( gcond == -1 ) return e_cur;
    ...
    double HMtheta1 = - logpost_v_vg( e_cur, l, T, alpha, li, ls ) + 0.5 * log(  G  ) + 0.5 * aux;
    double HMthetaf = 0.0;
    ...
    for (int jgl = 1 ; jgl < L + 1 ; jgl++ ){
    	...
    	gdpth2 = glogpost_v_vg( theta2a, l, inv_G, dG_v, T, alpha, li, ls );
    	gradHMCth2 = gdpth2 - 0.5 * nuH_v_vg(dG_v, inv_G, pa);
    
    	for(int jfix = 1 ; jfix < fixp + 1 ; jfix++ ){
    		pb = pa - 0.5 * eps * gradHMCth2;
     		gradHMCth2 = gdpth2 - 0.5 * nuH_v_vg( dG_v, inv_G, pb);
    	}
    
    	gcond = G_v_vg( theta2a, G, inv_G, dG_v, T, alpha, li, ls );
    	if( gcond == -1 ) return e_cur;

      gdmom1 = gradHmom_v_vg( inv_G, pb );
    	gdmom2 = gradHmom_v_vg( inv_G, pb );
    
    	for(int jfix = 1 ; jfix < fixp + 1 ; jfix++ ){
    		theta2b = theta2a + 0.5 * eps * gdmom1 + 0.5 * eps * gdmom2;
    		gcond = G_v_vg( theta2b, G, inv_G, dG_v, T, alpha, li, ls );
        if( gcond == -1 ) return e_cur;
    		gdmom2 = gradHmom_v_vg( inv_G, pb );
    	}
    
    	gdpth2 = glogpost_v_vg( theta2b, l, inv_G, dG_v, T, alpha, li, ls );
    	gradHMCth2 = gdpth2 - 0.5 * nuH_v_vg( dG_v, inv_G, pb);
    	pb = pb - 0.5 * eps * gradHMCth2;
    	pa = pb;
    	theta2a = theta2b;
    ...
    HMthetaf = - logpost_v_vg( theta2b, l, T, alpha, li, ls ) + 0.5 * log(  G  ) + 0.5 * aux;
    uMH = randu( distr_param( 0, 1 ) );
